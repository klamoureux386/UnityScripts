...
//SOURCE: http://thehiddensignal.com/unity-angle-of-sloped-ground-under-player/
...
public class GroundChecker : MonoBehaviour
{
    ...
    public void CheckGround(Vector3 origin)
    {
        ...
        //if raycast hits, slope is upward
        if (Physics.Raycast(origin + rayOriginOffset3, transform.TransformDirection(Vector3.forward), out slopeHit3, raycastLengthForward))
        {
            // Debug line to second hit point
            if (showDebug) { Debug.DrawLine(origin + rayOriginOffset3, slopeHit3.point, Color.red); }
        }

        //else slope is downward
        else {  
            groundSlopeAngle *= -1;
        }
